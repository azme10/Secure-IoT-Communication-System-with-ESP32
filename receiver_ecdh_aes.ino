...
void OnDataRecv(const esp_now_recv_info_t *info, const uint8_t *data, int len) {
  ...
  if (len == sizeof(ecdh_message_t)) {
    Serial.println("📥 Received ECDH public key from Sender");
    ...
    ret = mbedtls_ecp_point_write_binary(&grp, &our_public,
                                         ...
                                         &pub_olen, ecdh_msg.publicKey, 65);
    if (ret != 0) {
      Serial.printf("❌ Failed to export public key: %d\n", ret);
      return;
    }
    
    Serial.println("📤 Sending our ECDH public key back to Sender...");
    esp_now_send(senderAddress, (uint8_t*)&ecdh_msg, sizeof(ecdh_msg));
...
void setup() {
  ...
  Serial.println("🔧 Initializing ECDH-AES Receiver...");
  ...
  const char *pers = "ecdh_aes_receiver";
  ...
  Serial.println("✅ Receiver ECDH key pair generated");
...
void loop() {
  ...
  if (keyExchangeComplete && aesKeyReady) {
    ...
    sendEncryptedMessage("Hello from ECDH Receiver!");
    
    delay(5000);
    sendEncryptedMessage("Elliptic Curve cryptography is awesome!");
